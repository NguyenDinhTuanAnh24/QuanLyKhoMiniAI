...
export default function SalesPage({ onNavigate }) {
  ...
  const handleConfirmAction = () => {
    ...
    if (customerInfo.paymentMethod === 'Chuyển khoản') {
      const orderCode = `DH${new Date().toISOString().slice(2,10).replaceAll("-", "")}${String(Date.now()).slice(-4)}`;
      const cName = customerInfo.name.trim() || 'Khach le';
      const content = normalizeTransferText(`${cName} ${orderCode}`);
      
      setPendingOrderCode(orderCode);
      setTransferContent(content);

      const url = `https://img.vietqr.io/image/${BANK_ID}-${ACCOUNT_NO}-${TEMPLATE}.png?amount=${total}&addInfo=${encodeURIComponent(content)}&accountName=${encodeURIComponent(ACCOUNT_NAME)}`;
      setQrImageUrl(url);
      setQrError(false);
      setShowQRModal(true);
      return;
            ...
            <div className="flex-1 overflow-y-auto pr-2">
              <div className="flex justify-center mb-6 bg-slate-50 rounded-xl py-6 border border-slate-100">
                {qrImageUrl && !qrError ? (
                  <img 
                    src={qrImageUrl} 
                    alt="VietQR" 
                    className="w-56 h-56 object-contain rounded-xl shadow-sm bg-white p-2 border border-slate-200" 
                  ...
                  <div className="flex flex-col items-center justify-center text-slate-500 text-center px-4 w-56 h-56 bg-white rounded-xl shadow-sm border border-slate-200">
                    <span className="text-sm">Không thể tải mã QR.<br/>Vui lòng kiểm tra thông tin chuyển khoản bên dưới.</span>
                  </div>
                )}
              </div>
              ...
              <div className="bg-slate-50 border border-slate-100 p-5 rounded-xl text-sm space-y-3 mb-6">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-blue-600 text-lg leading-none">{formatCurrency(total)}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between border-t border-slate-200 pt-3 mt-3">
                  ...
                  <span className="font-medium text-slate-800">{pendingOrderCode}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-500">Nội dung:</span>
                  <span className="font-medium text-slate-800">{transferContent}</span>
                </div>
              </div>
            </div>
